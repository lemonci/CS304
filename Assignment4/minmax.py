class TicTacToe:
    ...
    def miniMax(self, alpha=-float('inf'), beta=float('inf'), player='o', depth=float('inf')):
        end = self.isEnd()
        if end:     return end
            ...
            for i in range(self.side):
                for j in range(self.side):
                    if self.board[i][j]=='.':
                        ...
                        val, _, _ = self.miniMax(alpha=alpha, beta=beta, player='o', depth=depth-1)
            ...
            for i in range(self.side):
                for j in range(self.side):
                    if self.board[i][j]=='.':
                        ...
                        val, _, _ = self.miniMax(alpha=alpha, beta=beta, player='x', depth=depth-1)
                        if val>nodeVal:
                            self.x, self.y = i, j
    ...
    def play(self, RecommendMoves=False, depth=None):
        ...
        while True:
            ...
            while True and self.player=='x':
                if RecommendMoves:
                    nodeVal, qx, qy = self.miniMax(player='x', depth=depth)
                ...
                # If it's AI's turn
                start = time.time()
                nodeVal, px, py = self.miniMax(player='o', depth=depth)
                print(self.x, self.y)
                print('---', px, py)
                end = time.time()
